...
var MedicoListComponent = /** @class */ (function () {
    function MedicoListComponent(formBuilder, router, medicoService) {
    ...
    ; // fim ngOnInit()
    MedicoListComponent.prototype.edit = function (medico) {
        //  [routerLink]="[medico.idMedico,'edit']"
        this.medicoService.setDados(medico);
        this.router.navigate(['/medico/' + medico.idMedico + '/edit']);
    };
    MedicoListComponent.prototype.delete = function (medico) {
        var resp = this.medicoService.deleteMedico(medico);
        var index = this.medicos.map(function (item) { return item.idMedico; }).indexOf(medico.idMedico);
